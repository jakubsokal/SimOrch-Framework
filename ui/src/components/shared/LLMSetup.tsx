...
const PROVIDERS = ['ollama', 'openai', 'anthropic', 'groq', 'gemini'] as const;
...
export const MODELS_BY_PROVIDER: Record<Provider, string[]> = {
  ...
  anthropic: ['claude-2', 'claude-instant-100k', 'claude-1', 'claude-1.3'],
...
const LLMSetup: FC<LLMSetupProps> = ({ config, onChange }) => {
  ...
  useEffect(() => {
    fetch('http://localhost:8000/initiate/env-keys')
      .then(r => r.json())
      .then(setEnvKeys)
      .catch(() => setEnvKeys({}));
  }, []);

  const providerHasEnvKey = envKeys[config.provider] ?? false;
  ...
  return (
    <div className="border-t border-gray-100 pt-4 space-y-4">
        ...
        <div className="flex flex-wrap gap-2">
          {PROVIDERS.map(p => (
            <SelectButton
              ...
              onClick={() => onChange({ ...config, provider: p, model: MODELS_BY_PROVIDER[p][0], api_key: '' })}
            />
          ))}
        </div>
      </div>
      ...
      <div className="space-y-1.5">
        ...
        {!providerHasEnvKey && config.provider !== 'ollama' && (
          <FormInput
            label="API KEY"
            type="password"
            placeholder="API Key"
